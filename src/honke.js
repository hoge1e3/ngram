// @hoge1e3/ngram
// honke is typescript version but different implementation (never tested).
...
export class LetterIndex {
    ...
    /**
     * Traverse all words in this index tree.
     * @param prefix Prefix for words to be yielded.
     */
    *words(prefix=""):Generator<string>{
        const letters=[
            ...this.map
        ...
        for(let {letter,index} of letters){
            yield* index.words(prefix+(letter===EOF?"":letter));
        }
    }
    /* example:
        `word`="something" 
    ...
    */
    walk(word:string):WalkResult{
        ...
        // index is DocumentIndex && rest => depends on index(document) content
